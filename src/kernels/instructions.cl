...
#define FLAG_TRAN_LINEAR_DP (0x1 << 5)
#define FLAG_TRAN_LINEAR_NO_PASS (0x1 << 6)
...
inline void edge(particle_work_t *part_data, const flags_t flags,
  ...
  part_data->particle.py -= y0 * curvature * _tan(psi);
}
...
inline void wire(particle_work_t *part_data, const flags_t flags,
  ...
  const float_t alpha = _atan2(wire_y - y0, wire_x - x0);
  const float_t B = _divide(k, _hypot(wire_x - x0, wire_y - y0));

  part_data->particle.px += B * _cos(alpha);
  part_data->particle.py += B * _sin(alpha);
}
...
inline void cavity(particle_work_t *part_data, const flags_t flags,
  ...
  part_data->particle.dp += field * _sin((2.0 * M_PI) * (lag - omega * z0));
  update_oodppo(part_data);
}

// NOTE: This uses a different flag notation
// Generic linear xypxpy phase space transform plus shift and optional dp input.
inline void tran_linear(particle_work_t *part_data, const flags_t flags,
                        const intarg_t args0, const intarg_t args1,
                        __local const float_t *args0_arr,
                        __local const float_t *args1_arr) {
  UNUSED(args1_arr)
  ...
  if (flags & FLAG_TRAN_LINEAR_DP) {
    part_data->particle.x += dp0 * args0_arr[slice + 0];
    part_data->particle.y += dp0 * args0_arr[slice + 1];
    part_data->particle.px += dp0 * args0_arr[slice + 2];
    part_data->particle.py += dp0 * args0_arr[slice + 3];
    slice += 4;
  }
